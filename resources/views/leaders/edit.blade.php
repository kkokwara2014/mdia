        ...
        <form method="POST" action="{{ route('leaders.update', $leader) }}" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <select name="role_uuid" class="form-select @error('role_uuid') is-invalid @enderror" required>
                    <option value="" disabled @selected(! old('role_uuid', $leader->role?->uuid))>Select a role</option>
                    @foreach($roles as $role)
                        <option value="{{ $role->uuid }}" @selected(old('role_uuid', $leader->role?->uuid) === $role->uuid)>{{ $role->name }}</option>
                    @endforeach
                </select>
                @error('role_uuid')
                    <div class="invalid-feedback">{{ $message }}</div>
